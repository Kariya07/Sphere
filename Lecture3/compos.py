...
def main():
    ...
    search_params(train_data, test_data)
...
def f1_score(test_data, prediction):
    labels = unique_labels(test_data[0::, 0], prediction)
    _, _, f1, _ = precision_recall_fscore_support(test_data[0::, 0], prediction,
                                                  labels=labels,
                                                  average=None)
    f1_macro = 0
    for i, _ in enumerate(labels):
        f1_macro += f1[i]
    return f1_macro / labels.size

def search_params(train_data,  test_data):
    estimators = [20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80]
    res1 = []
    res2 = []
    res3 = []
    time1 = []
    ...
    for n_estimators in tqdm(estimators):
        ...
        gbm_classifier = GradientBoostingClassifier(n_estimators=n_estimators)
        
        start = time.time()
        forest = forest.fit(train_data[0::, 1::], train_data[0::, 0])
        res1 += [f1_score(test_data, forest.predict(test_data[0::, 1::]))]
        end = time.time()
        time1 += [end-start]
        
        start = time.time()
        adaboost_classifier.fit(train_data[0::, 1::], train_data[0::, 0])
        res2 += [f1_score(test_data, adaboost_classifier.predict(test_data[0::, 1::]))]
        end = time.time()
        time2 += [end-start]
        
        start = time.time()
        gbm_classifier.fit(train_data[0::, 1::], train_data[0::, 0])
        res3 += [f1_score(test_data, gbm_classifier.predict(test_data[0::, 1::]))]
        end = time.time()
        time3 += [end-start]
    pl.title('F1 score')
    pl.plot(estimators, res1, label='RandomForest')
    pl.plot(estimators, res2, label='AdaBoost')
    pl.plot(estimators, res3, label='GradientBoosting')
    ...
    pl.title('Wall time')
    pl.plot(estimators, time1, label='RandomForest')
    pl.plot(estimators, time2, label='AdaBoost')
    pl.plot(estimators, time3, label='GradientBoosting')
    pl.xlabel('n_estimators')
    pl.ylabel('time')
    pl.legend()
    pl.show()
